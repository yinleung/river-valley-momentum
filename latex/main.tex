...
In the closed loop, momentum shifts the hill stability threshold to $\eta\lambda<2\Teff$, sets the stationary hill variance to $\eta\sigma^2/(\lambda(2-\eta\lambda/\Teff))$, and makes the stationary hill loss strictly decreasing in $\beta$ with maximal relative reduction exactly $\eta\lambda/2$---a claim about the EMA convention at fixed $\eta$: at fixed $\eta_{\mathrm{HB}}$ the sweep reverses at small $\eta_{\mathrm{HB}}\lambda$, with closed-form optimum $\max(0,\sqrt{\eta_{\mathrm{HB}}\lambda}-1)$, and we state both as theorems.
The closed-loop forced response then separates what filtering does at the trajectory level: a Nyquist disturbance is removed at gain $\propto1/\Teff$, a passband bias is displaced by $1/\lambda$ at every $\beta$, and a tone at the loop's own underdamped frequency is amplified without bound as $\beta\to1$---the benefit is a $\beta$ window, not a monotone preference for $\beta\to1$.
A curved-valley reduction carries the loop to bending rivers in labeled strengths---exact for the linear floor, where it decouples into two straight loops at the Hessian eigenvalues; quasi-static, with a stated validity margin, for general floors---and its frozen stability diagnostic matches the measured confinement floors to grid resolution.
For the matrix polar factor used by Muon we keep buffer and update apart: filter-first theorems in deterministic-Nyquist, band-limited, and band-energy forms control the buffer feeding the map; a necessity example shows the exact polar of a rank-deficient signal transfers none of that control; and two update-level theorems recover the transfer at the same $1/\Teff$ rate---for the exact polar under a full-rank signal, and for the finite Newton--Schulz operator practical Muon deploys, at an explicit Lipschitz price.
Controlled experiments measure the predictions: the hill loss falls with $\beta$ by the predicted margin and only in the predicted regime; forced-disturbance controls follow the parameter-free gain guides over nearly three orders of magnitude, including the predicted resonant harm; a real network at the edge of stability shows the Nyquist oscillation emerging only at valley confinement; and in mini-batch transformer training, where the momentum benefit grows with the learning rate---to $12\%$ at the largest rate every arm survives, with $\beta=0$ failing outright at the rate above while momentum trains---a large-batch decomposition places the high-frequency content in the mean-loss gradient at the visited iterates, concentrated on the top curvature directions---hill oscillation, not sampling noise---with the filter-first ordering persisting at that scale.
Negative results are reported alongside: deterministic ringing at large $\beta$, a mechanism score that does not rank $\beta$ at transformer scale, and the update-level knee of the deployed Newton--Schulz operator left unmeasured.
\end{abstract}
...
\item We characterize what the closed loop does to \emph{every} frequency: the forced-response gain $\lvert G_\beta(\omega)\rvert$ (\Cref{prop:forced}) removes Nyquist disturbances at rate $1/\Teff$, leaves passband bias at the $\beta$-free displacement $1/\lambda$, and amplifies tones at the loop's own underdamped frequency without bound as $\beta\to1$---three falsifiable controls that E13 measures on parameter-free guides, in $28$ of $28$ cells.
A curved-valley reduction (\Cref{prop:curved}) carries the closed loop to bending rivers in three labeled strengths: an exact one-step identity; an exact solution for the linear floor, which decouples into \emph{two} straight-valley loops at the Hessian eigenvalues and exposes the frozen-coefficient formulas as its $\mu\to0$ limit; and a quasi-static reduction for general floors at the local sharpness $\lambda(1+f'^2)$ with a geometric passband forcing, whose stated validity margin is the filter memory against the bend.
The frozen stability diagnostic matches E10's measured confinement floors to grid resolution, and the $\beta$-free tracking offset is the geometric cost curvature exacts at every momentum.
\item We separate what filtering does to the buffer from what reaches the update.
The buffer theorems: the buffer's Nyquist disturbance dies at rate $1/\Teff$ (\Cref{thm:filterfirst})---deterministically, faster than the $\Teff^{-1/4}$ stochastic rate of \citet{li2026denoise}---with a band-limited generalization (\Cref{thm:bandlimited}) contracting any high-band disturbance by $\rhohigh(\beta)$ while tracking a slowly varying signal, and a band-energy corollary (\Cref{cor:bandenergy}) whose hypothesis is the disturbance stream's measured high-band energy fraction---the quantity HFER estimates.
The update theorems: the recovery transfers to the update itself, at the same $1/\Teff$ rate, for the exact polar under a full-rank signal (\Cref{thm:polarupdate}) and---with no rank hypothesis---for the finite Newton--Schulz operator practical Muon deploys, at the explicit Lipschitz price $L_h\approx485$ (\Cref{thm:nsupdate}); the obstruction in between---exact polar erases amplitudes---is stated as a necessity example (\Cref{prop:polaronly}) fixing those two hypotheses as the admissible ones.
\item We test the predictions in controlled settings (\Cref{sec:experiments}; \Cref{tab:map} maps results to tests, tags each row's statement class, and marks the one untested row---the update theorems' knee), led by a direct optimization-quality sweep in which the measured hill loss falls with $\beta$ by the predicted $\eta\lambda/2$ and only in the predicted regime (E9), with the $\beta$ window traced along both of its axes (conditioning and river curvature; E10) and the mechanism exposed to falsification by forced-disturbance controls whose outcome must follow frequency content, not $\beta$ (E13).
...
\end{itemize}
\paragraph{Statement classes.} Every labeled claim in this paper carries one of four strengths: \emph{exact} (closed-form statements proved as displayed: the straight valley, the buffer theorems, the linear-floor curved valley, the update theorems); \emph{quasi-static} (frozen-coefficient reductions whose freezing hypotheses are stated and instrumented: \Cref{prop:curved}(c)); \emph{reading} (remarks that collect proved constants into an interpretation); and \emph{measured} (claims that exist only as experimental outcomes).
\Cref{tab:map} tags the class of each result it maps to an experiment, and a statement of one class is never quoted as a member of a stronger one.
...
\citet{lessard2016analysis} treat momentum's convergence rate as a robust-control problem on the same second-order system---the acceleration question this paper keeps separate from filtering (\Cref{sec:discussion})---and the noisy quadratic model of \citet{zhang2019noisy} is the closest precedent for our validation discipline: exact quadratic-model predictions, read against network training for shape and ordering rather than constants.
...
$c$; $\nu_1,\nu_2$ & linear-floor slope; its Hessian eigenvalues & \Cref{prop:curved}\\
...
$\Dcal_{\mathrm{NS}}$; $K$; $\psi$ & deployed NS operator; iteration count; NS polynomial & \Cref{eq:deployed}\\
$h_{\mathrm{NS}}$; $L_h$; $\kappa$; $\varepsilon_{\mathrm{NS}}$ & scalar NS response; its slope bound; Frobenius floor; NS guard & \Cref{eq:deployed}; \Cref{thm:nsupdate}\\
\bottomrule
\end{tabular}
\end{table}
...
Consequently momentum raises the river-to-hill energy ratio by a factor up to $((1+\beta)/(1-\beta))^2$, in the open-loop, near-pure-tone limit; it is not a closed-loop guarantee (\Cref{sec:loop}).
\end{theorem}
...
For the curved valley $L(x,y)=\phi(x)+\tfrac{\lambda}{2}(y-f(x))^2$ with river floor $y=f(x)$, the same separation holds locally with two geometric corrections---a sharpened hill and a low-frequency forcing from the bend---made quantitative in \Cref{prop:curved} below (exactly for the linear floor, quasi-statically for general floors), once the closed loop is in place.
...
The same AR(2) is the heavy-ball second-order system whose rate-optimal tuning is the subject of the acceleration literature \citep{polyak1964momentum,lessard2016analysis}; convergence rate is not this paper's question, and open-loop filtering supplies none at DC (\Cref{sec:discussion}).
...
All constants are in the EMA normalization; \Cref{cor:hb} restates stability and loss at fixed $\eta_{\mathrm{HB}}$, where the sweep direction inverts at small $\eta_{\mathrm{HB}}\lambda$.
...
The normalization scope of these statements takes theorem-level form.

\begin{corollary}[Heavy-ball normalization]
\label{cor:hb}
Fix $\eta_{\mathrm{HB}}>0$ and sweep $\beta$ under the heavy-ball convention of \Cref{sec:ema}, i.e.\ run \Cref{eq:closedloop} at $\eta=\eta_{\mathrm{HB}}/(1-\beta)$.
All formulas of \Cref{prop:stability,prop:variance,cor:hillloss,prop:forced} apply verbatim at that $\eta$, and read at fixed $\eta_{\mathrm{HB}}$:
\begin{enumerate}
\item[(a)] (stability) the loop is stable iff $\eta_{\mathrm{HB}}\lambda<2(1+\beta)$; for $\eta_{\mathrm{HB}}\lambda\ge2$ this requires $\beta>\eta_{\mathrm{HB}}\lambda/2-1$;
\item[(b)] (stationary hill loss) $\mathbb{E}[\tfrac{\lambda}{2}y_\infty^2]=\eta_{\mathrm{HB}}\sigma^2/\bigl(2\,(1-\beta)\,(2-\eta_{\mathrm{HB}}\lambda/(1+\beta))\bigr)$ on the stable range, minimized at $\beta^\star=\max\bigl(0,\sqrt{\eta_{\mathrm{HB}}\lambda}-1\bigr)$: strictly increasing in $\beta$ for $\eta_{\mathrm{HB}}\lambda<1$, U-shaped with interior minimum for $1<\eta_{\mathrm{HB}}\lambda<4$;
\item[(c)] (sweep direction) the direction of the $\beta$-sweep at fixed step parameter is convention-dependent---decreasing under EMA normalization (\Cref{cor:hillloss}), increasing under heavy-ball normalization whenever $\eta_{\mathrm{HB}}\lambda\le1$---while the guides and the stability boundary are convention-invariant.
\end{enumerate}
\end{corollary}

\begin{proof}
Substitute $\eta=\eta_{\mathrm{HB}}/(1-\beta)$ into \Cref{prop:stability} ($\eta\lambda<2\Teff\Leftrightarrow\eta_{\mathrm{HB}}\lambda<2(1+\beta)$) and into \Cref{cor:hillloss}, using $(1-\beta)\Teff=1+\beta$.
The $\beta$-derivative of the denominator $(1-\beta)(2-\eta_{\mathrm{HB}}\lambda/(1+\beta))$ is $-2+2\eta_{\mathrm{HB}}\lambda/(1+\beta)^2$, negative for all $\beta\ge0$ iff $\eta_{\mathrm{HB}}\lambda\le1$ and vanishing at $(1+\beta^\star)^2=\eta_{\mathrm{HB}}\lambda$ otherwise; $\beta^\star$ lies in the stable range since $\sqrt{\eta_{\mathrm{HB}}\lambda}>\eta_{\mathrm{HB}}\lambda/2$ for $\eta_{\mathrm{HB}}\lambda<4$.
\end{proof}

Under scale-invariant direction maps (polar, normalized updates) the two conventions coincide up to the step size; for SGDM trajectories they do not, and which convention is held fixed is part of every claim below.
E9's normalization ablation (\Cref{sec:e9}) measures the increasing sweep at $\eta_{\mathrm{HB}}\lambda=0.6$ and the decreasing EMA sweep from the same $\beta=0$ cell, both on their composed guides.
...
Three statements of three strengths carry it: an exact one-step identity for any floor, an exact solution for the linear floor, and a quasi-static reduction for general floors whose two hypotheses are stated rather than implied.
...
\item[(b)] (exact solution, linear floor) for $f(x)=cx$ with $c\ne0$ and $\phi(x)=\tfrac{\mu}{2}(x-x^\star)^2$, $0<\mu<\lambda$, under EMA momentum with per-coordinate white noise of variance $\sigma^2$: let $\nu_1>\nu_2>0$ be the eigenvalues of the constant loss Hessian $\nabla^2L=\bigl(\begin{smallmatrix}\mu+\lambda c^2&-\lambda c\\-\lambda c&\lambda\end{smallmatrix}\bigr)$---real and distinct, with $\nu_1+\nu_2=\mu+\lambda_{\mathrm{loc}}$, $\nu_1\nu_2=\mu\lambda$, and $\nu_1\ge\max(\lambda_{\mathrm{loc}},\mu)$---and $\hat e_1,\hat e_2$ its orthonormal eigenvectors.
The loop decouples exactly into two independent copies of \Cref{eq:closedloop}, mode $i$ at curvature $\nu_i$ with noise variance $\sigma^2$.
Consequently it is stable iff $\eta\nu_1<2\Teff$, $\mathbb{E}[d_\infty]=0$, and, with the weights $b_i=\langle(-c,1),\hat e_i\rangle$ (so $b_1^2+b_2^2=1+c^2$),
  \[
  \operatorname{Var}(d_\infty)
  =b_1^2\,\frac{\eta\sigma^2}{\nu_1\bigl(2-\eta\nu_1/\Teff\bigr)}
  +b_2^2\,\frac{\eta\sigma^2}{\nu_2\bigl(2-\eta\nu_2/\Teff\bigr)},\qquad
  b_1^2=\frac{(\lambda_{\mathrm{loc}}-\nu_2)^2}{(\lambda-\nu_2)(\nu_1-\nu_2)},\quad
  b_2^2=\frac{(\nu_1-\lambda_{\mathrm{loc}})^2}{(\nu_1-\lambda)(\nu_1-\nu_2)},
  \]
with no cross term.
If instead the river profile has constant slope, $\phi'\equiv\mathrm{const}$ (any $c$), then $d_t$ decouples exactly at sharpness $\lambda_{\mathrm{loc}}$ with noise variance $\sigma^2(1+c^2)$ and constant forcing, and for every stable $\beta$
  \[
  \mathbb{E}[d_\infty]=\frac{c\,\phi'}{\lambda_{\mathrm{loc}}},\qquad
  \operatorname{Var}(d_\infty)
  =\frac{\eta\sigma^2}{\lambda\,\bigl(2-\eta\lambda_{\mathrm{loc}}/\Teff\bigr)} ;
  \]
\item[(c)] (quasi-static reduction, general $f$) if $f'$ is frozen at $f'_0$ over the filter memory \emph{and} the river path $\{x_j\}$ is treated as an exogenous deterministic input---the two hypotheses of this part---then $d_t$ obeys \Cref{eq:closedloop} with $\lambda\to\lambda_{\mathrm{loc}}=\lambda(1+f_0'^2)$, forcing $u_j=-f'_0\,\phi'(x_j)$, and noise variance $\sigma^2(1+f_0'^2)$, up to the second-order remainder $-\tfrac12 f''(\zeta_t)(\eta m^x_t)^2$ of (a).
\end{enumerate}
\end{proposition}

\begin{proof}
(a) $d_{t+1}=y_t-\eta\lambda d_t-f(x_t+\Delta x_t)$; Taylor with Lagrange remainder and $y_t-f(x_t)=d_t$ collect to the display.
(b) $L$ is a quadratic form with constant Hessian; the discriminant of its characteristic polynomial is $(\lambda_{\mathrm{loc}}-\mu)^2+4c^2\lambda\mu>0$, giving the stated sum, product, and $\nu_1\ge\max(\lambda_{\mathrm{loc}},\mu)$.
The joint update on $(w_t,m_{t-1})$ has blocks that are polynomials in $\nabla^2L$, so the orthonormal eigenbasis decouples it into two scalar loops, mode $i$ obeying \Cref{eq:closedloop} at curvature $\nu_i$; the rotated noise $Q^\top\xi$ is again iid with variance $\sigma^2$ per coordinate, so the modes are independent.
Stability is \Cref{prop:stability} per mode, binding at $\nu_1$; the mean recursion is noise-free and contracts to the minimum, giving $\mathbb{E}[d_\infty]=0$.
Writing $d=\langle(-c,1),w-w^\star\rangle=\sum_ib_ip_i$ in the modal coordinates $p_i$, independence gives $\operatorname{Var}(d_\infty)=\sum_ib_i^2\operatorname{Var}(p_{i,\infty})$ with \Cref{prop:variance} per mode; the weight formulas follow from the eigenvector components $\hat e_i\propto(\lambda c,\nu_{3-i}-\lambda)$ and the identity $(\nu_1-\lambda)(\lambda-\nu_2)=c^2\lambda^2$, and $b_1^2+b_2^2=\lVert(-c,1)\rVert^2$.
For the constant-slope profile the $d$-combination of the gradient stream is $\lambda_{\mathrm{loc}}d_j-c\phi'+(\xi^y_j-c\xi^x_j)$, with no $x$-coupling: \Cref{eq:closedloop} at $(\lambda_{\mathrm{loc}},\sigma^2(1+c^2))$ under constant forcing, whose fixed point and variance are \Cref{prop:forced}(a) and \Cref{prop:variance}, the $(1+c^2)$ factors cancelling except inside the denominator.
(c) The update gives $d_{t+1}=d_t-\eta\,(m^y_t-f'(x_t)\,m^x_t)-\tfrac12 f''(\zeta_t)(\eta m^x_t)^2$, and with $f'$ frozen and $\{x_j\}$ exogenous the buffer combination is $m^y-f'_0m^x=\mathrm{EMA}_j\bigl[\lambda_{\mathrm{loc}}d_j-f'_0\phi'(x_j) +(\xi^y_j-f'_0\xi^x_j)\bigr]$, which is \Cref{eq:closedloop} for $d$ with the stated parameters ($\operatorname{Var}(\xi^y-f'_0\xi^x)=\sigma^2(1+f_0'^2)$) plus the retained remainder.
\end{proof}

\begin{remark}[Frozen coefficients against the exact solution]
\label{rem:frozenexact}
Part (c) freezes two things part (b) does not: the local sharpness and the stochasticity of the river coordinate.
On the linear floor the price is explicit and $\Ocal(\mu)$: as $\mu\to0$ at fixed $c$, $\nu_1\to\lambda_{\mathrm{loc}}$, $b_2^2\to0$, and (b)'s variance converges to (c)'s value $\eta\sigma^2/(\lambda(2-\eta\lambda_{\mathrm{loc}}/\Teff))$ with first-order correction $\mu\,c^2\eta^3\sigma^2/\bigl(2(2\Teff-\eta\lambda_{\mathrm{loc}})^2\bigr)$.
The frozen stability threshold has the same status: $\eta\lambda_{\mathrm{loc}}<2\Teff$ is necessary but not sufficient for (b)'s loop, whose exact boundary is $\eta\nu_1<2\Teff$ with $\nu_1=\lambda_{\mathrm{loc}}+\mu c^2/(1+c^2)+\Ocal(\mu^2/\lambda)$.
On a general floor, pointwise frozen stability is therefore a hypothesis-level diagnostic, not a trajectory theorem; a global curved-valley statement remains open (\Cref{sec:discussion}).
Part (c)'s freezing is admissible while the coefficient drift over one filter memory is small---$\Teff\,\vriv\,\lvert f''\rvert\lesssim\max(1,\lvert f'\rvert)$, i.e.\ $k\vriv\,\Teff\lesssim1$ on the sinusoidal floor---the hypothesis E10 measures against.
\end{remark}

Two consequences of the quasi-static part (c) carry the curved-valley measurements.
(i) \emph{Confinement floor}: frozen stability requires $\eta\lambda(1+f'^2)<2\Teff$ pointwise, worst case $\eta\lambda(1+(a_{\mathrm{b}}k)^2)<2\Teff$ on $f(x)=a_{\mathrm{b}}\sin(kx)$, so the smallest admissible $\beta$ rises with the bend through the slope range $a_{\mathrm{b}}k$; this diagnostic---necessary, not sufficient, by \Cref{rem:frozenexact}---predicts E10's measured confinement floors exactly in $7$ of $10$ $(\eta\lambda,k)$ cells and to one grid step in the remaining $3$ (\Cref{sec:e10}), deviations in both directions.
...
Where part (c)'s drift hypothesis fails the buffer lags the bend---the top edge of the $\beta$ window, whose closed-loop softening by river-speed collapse E10 measures.
E8's bend correction $\lambda(1+\mathbb{E}[f'^2])$ is (c) read in expectation.
...
The window---hence the range of well-performing $\beta$---widens with the valley conditioning $\lambda/\mu$, and on a curved river it is squeezed from below as the bend sharpens the local hills (the frozen diagnostic $\eta\lambda(1+f'^2)<2\Teff$ of \Cref{prop:curved}; E10 measures both axes).
\end{remark}
...
The results come at two levels, kept apart throughout: \emph{buffer theorems} (\Cref{sec:buffer}) control $\Mbf_t$, the input to the map, exactly; \emph{update theorems} (\Cref{sec:update}) control the map's output, under the hypotheses that a necessity example shows cannot be dropped.

\subsection{Buffer theorems}
\label{sec:buffer}

The buffer analysis is exact.

\begin{theorem}[Deterministic filter-first: the buffer]
...
\subsection{From the buffer to the update}
\label{sec:update}

The theorems above stop at the map's input, and for the exact polar factor that is a real boundary: $\polar(\cdot)$ flattens every nonzero singular value to $1$, erasing the amplitude information the buffer bounds control.
Whether the buffer's recovery transfers to the update is a separate question with a sharp answer: no in general (\Cref{prop:polaronly}); yes, at the buffer rate, once either the signal has full rank (\Cref{thm:polarupdate}) or the map is the finite Newton--Schulz iteration Muon actually deploys (\Cref{thm:nsupdate}).

\begin{proposition}[Exact polar transfers no amplitude]
\label{prop:polaronly}
Take $\Sbf=\sigma_S\,\ubf\vbf^\top$ and $\Abf=\alpha\,\ubf'\vbf'^\top$ with $\ubf\perp\ubf'$, $\vbf\perp\vbf'$, $\alpha>\sigma_S>0$, and read $\polar(\cdot)$ on a rank-deficient input in the partial-polar sense $\sum_{\sigma_i>0}\ubf_i\vbf_i^\top$ (the unitary polar factor is not unique there).
Then for every $t$ and every $\beta\in[0,1)$,
\[
\polar(\Mbf_t)=\ubf\vbf^\top+\sign(\varepsilon_t)\,\ubf'\vbf'^\top,
\qquad
\lVert\polar(\Mbf_t)-\polar(\Sbf)\rVert_F=1 :
\]
the buffer disturbance dies at rate $1/\Teff$ (\Cref{thm:filterfirst}) while the update error does not move, so no bound on the exact-polar update in terms of the disturbance amplitude alone is possible for a rank-deficient signal---equivalently, the singular map $\sigma\mapsto\sign(\sigma)$ has no finite Lipschitz constant on any interval containing the occupied value $0$.
On the same instance, $\polar(\Gbf_t)$ has top singular subspace $\mathrm{span}(\ubf')$ at every $t$: without the filter, even the input's singular-value ordering misidentifies the signal.
\end{proposition}

\begin{proof}
$\Mbf_t=(1-\beta^t)\sigma_S\,\ubf\vbf^\top+\varepsilon_t\alpha\,\ubf'\vbf'^\top$ has orthogonal singular pairs, so its partial polar is the displayed two-term sign sum, and the difference from $\polar(\Sbf)=\ubf\vbf^\top$ is $\pm\ubf'\vbf'^\top$, of Frobenius norm $1$.
$\Gbf_t$ has pairs $(\sigma_S,\ubf,\vbf)$ and $(\alpha,\ubf',\vbf')$, ordered by $\alpha>\sigma_S$.
\end{proof}

The example fixes the two admissible hypotheses, and each yields an update-level theorem.

\begin{theorem}[Full-rank polar update]
\label{thm:polarupdate}
In the setting of \Cref{thm:filterfirst}, assume additionally $r=d_2$, i.e.\ $\sigma_{\min}(\Sbf)=\sigma_r>0$ (WLOG $d_1\ge d_2$).
Whenever $(1-\beta^t)\sigma_r>\lvert\varepsilon_t\rvert\,\lVert\Abf\rVert_2$,
\[
\lVert\polar(\Mbf_t)-\polar(\Sbf)\rVert_F
\;\le\;\frac{2\,\lvert\varepsilon_t\rvert\,\lVert\Abf\rVert_F}{\sigma_{\min}(\Mbf_t)+(1-\beta^t)\sigma_r}
\;\le\;\frac{2\,\lvert\varepsilon_t\rvert\,\lVert\Abf\rVert_F}{2(1-\beta^t)\sigma_r-\lvert\varepsilon_t\rvert\,\lVert\Abf\rVert_2},
\]
so the update error dies at the buffer rate: as $t\to\infty$ it is at most $\lVert\Abf\rVert_F\,\bigl(1+\Ocal(1/\Teff)\bigr)/(\sigma_r\Teff)$, while polar-only obeys only the same bound with $\varepsilon_t$ replaced by $1$.
\end{theorem}

\begin{proof}
$\polar(c\Mbf)=\polar(\Mbf)$ for $c>0$, so $\polar(\Sbf)=\polar((1-\beta^t)\Sbf)$.
Apply the polar-factor perturbation bound of \citet{li1995polar} (also \citealp[Ch.~8]{higham2008functions}), $\lVert\polar(\Bbf_1)-\polar(\Bbf_2)\rVert_F\le2\,\lVert\Bbf_1-\Bbf_2\rVert_F/(\sigma_{\min}(\Bbf_1)+\sigma_{\min}(\Bbf_2))$ for full-column-rank arguments, to $\Bbf_1=\Mbf_t$ and $\Bbf_2=(1-\beta^t)\Sbf$ with $\lVert\Mbf_t-(1-\beta^t)\Sbf\rVert_F=\lvert\varepsilon_t\rvert\,\lVert\Abf\rVert_F$; Weyl's inequality bounds $\sigma_{\min}(\Mbf_t)\ge(1-\beta^t)\sigma_r-\lvert\varepsilon_t\rvert\,\lVert\Abf\rVert_2$.
\end{proof}

Practical Muon does not run the exact polar factor.
It runs the \emph{deployed Newton--Schulz operator}
\begin{equation}
\label{eq:deployed}
\Dcal_{\mathrm{NS}}(\Mbf)=\mathrm{NS}_K\Bigl(\tfrac{\Mbf}{\lVert\Mbf\rVert_F+\varepsilon_{\mathrm{NS}}}\Bigr),\qquad
\mathrm{NS}(\Xbf)=\psi_1\Xbf+\psi_3\,\Xbf(\Xbf^\top\Xbf)+\psi_5\,\Xbf(\Xbf^\top\Xbf)^2,
\end{equation}
the $K$-fold composition of one odd matrix polynomial on the Frobenius-normalized buffer; the deployed instance is $K=5$, $(\psi_1,\psi_3,\psi_5)=(3.4445,\,-4.7750,\,2.0315)$, $\varepsilon_{\mathrm{NS}}=10^{-7}$ \citep{jordan2024muon}.
Because the polynomial is odd, $\mathrm{NS}_K(\Xbf)=\Ubf\,h_{\mathrm{NS}}(\Sigmabf)\,\Vbf^\top$ in the input's own singular frame, with scalar response $h_{\mathrm{NS}}=\psi^{\circ K}$ for $\psi(\sigma)=\psi_1\sigma+\psi_3\sigma^3+\psi_5\sigma^5$: $h_{\mathrm{NS}}(0)=0$, and $h_{\mathrm{NS}}'(0)=\psi_1^K\approx485$---a finite slope at zero where the exact polar's is infinite.
That finite slope is the entire content of the next result.

\begin{lemma}[Odd matrix polynomials are scalar-Lipschitz in Frobenius norm]
\label{lem:oddlip}
Let $h$ be an odd real polynomial with $\lvert h'\rvert\le L$ on $[-s,s]$, acting on $\Xbf\in\Rbb^{d_1\times d_2}$ through its singular values, $h(\Ubf\Sigmabf\Vbf^\top)=\Ubf\,h(\Sigmabf)\,\Vbf^\top$ (equal to the odd matrix polynomial in $\Xbf$).
Then $\lVert h(\Xbf)-h(\Ybf)\rVert_F\le L\,\lVert\Xbf-\Ybf\rVert_F$ whenever $\lVert\Xbf\rVert_2,\lVert\Ybf\rVert_2\le s$.
(Proof in \Cref{app:proofs}, via the Hermitian dilation and the classical divided-difference bound.)
\end{lemma}

\begin{theorem}[Deployed Newton--Schulz update]
\label{thm:nsupdate}
Let $L_h=\sup_{[0,1]}\lvert h_{\mathrm{NS}}'\rvert$ ($=\psi_1^K\approx485$ for the deployed instance).
For any $\Mbf,\Xbf$ with $\min(\lVert\Mbf\rVert_F,\lVert\Xbf\rVert_F)\ge\kappa>0$,
\[
\lVert\Dcal_{\mathrm{NS}}(\Mbf)-\Dcal_{\mathrm{NS}}(\Xbf)\rVert_F\;\le\;\frac{2\,L_h}{\kappa+\varepsilon_{\mathrm{NS}}}\,\lVert\Mbf-\Xbf\rVert_F .
\]
In the setting of \Cref{thm:filterfirst}---with no rank hypothesis on $\Sbf$---take $\Xbf_t=(1-\beta^t)\Sbf$ and $\kappa_t=(1-\beta^t)\lVert\Sbf\rVert_F-\lvert\varepsilon_t\rvert\,\lVert\Abf\rVert_F$: whenever $\kappa_t>0$,
\[
\lVert\Dcal_{\mathrm{NS}}(\Mbf_t)-\Dcal_{\mathrm{NS}}(\Xbf_t)\rVert_F
\;\le\;\frac{2\,L_h\,\lvert\varepsilon_t\rvert\,\lVert\Abf\rVert_F}{\kappa_t+\varepsilon_{\mathrm{NS}}}
\;\longrightarrow\;\frac{2\,L_h\,\lVert\Abf\rVert_F}{\lVert\Sbf\rVert_F\,\Teff}\,\bigl(1+o(1)\bigr)\quad(t\to\infty):
\]
the buffer's $1/\Teff$ recovery transfers to the update Muon actually computes, at the explicit price $L_h$.
\end{theorem}

\begin{proof}
Normalization: $\nu(\Mbf)=\Mbf/(\lVert\Mbf\rVert_F+\varepsilon_{\mathrm{NS}})$ satisfies $\lVert\nu(\Mbf)-\nu(\Xbf)\rVert_F\le\tfrac{2}{\kappa+\varepsilon_{\mathrm{NS}}}\lVert\Mbf-\Xbf\rVert_F$ on $\{\lVert\cdot\rVert_F\ge\kappa\}$: write $\nu(\Mbf)-\nu(\Xbf)=\tfrac{\Mbf-\Xbf}{\lVert\Mbf\rVert_F+\varepsilon_{\mathrm{NS}}}+\Xbf\,\tfrac{\lVert\Xbf\rVert_F-\lVert\Mbf\rVert_F}{(\lVert\Mbf\rVert_F+\varepsilon_{\mathrm{NS}})(\lVert\Xbf\rVert_F+\varepsilon_{\mathrm{NS}})}$, bound the second factor's numerator by $\lVert\Mbf-\Xbf\rVert_F$ (reverse triangle inequality) and $\lVert\Xbf\rVert_F/(\lVert\Xbf\rVert_F+\varepsilon_{\mathrm{NS}})$ by $1$.
Polynomial: \Cref{lem:oddlip} at $s=1$, since $\lVert\nu(\cdot)\rVert_2\le\lVert\nu(\cdot)\rVert_F<1$.
Compose; in the displayed instance $\lVert\Mbf_t\rVert_F\ge\kappa_t$ and $\lVert\Xbf_t\rVert_F\ge\kappa_t$ by the triangle inequality, and $\lvert\varepsilon_t\rvert\to1/\Teff$.
\end{proof}

\begin{remark}[The constant and the knee]
\label{rem:knee}
$L_h$ is not decorative.
$\mathrm{NS}_K$ is built to amplify small singular values back to order one, so a normalized disturbance component above ${\sim}1/L_h$ is re-amplified, and the update error decays slowly in $\Teff$ until the disturbance amplitude crosses the knee ${\sim}(\kappa+\varepsilon_{\mathrm{NS}})/(2L_h)$; below the knee the map is in its linear zone $h_{\mathrm{NS}}(\sigma)\approx\psi_1^K\sigma$ and the bound is tight to a small factor.
Equivalently, the update-level recovery engages once $\Teff\gtrsim L_h\,\lVert\Abf\rVert_F/\lVert\Sbf\rVert_F$.
The knee is the update-level quantity a practical Muon experiment must probe; the comparisons in this paper (E5, E7, E11) test pipeline orderings, not the knee (\Cref{sec:discussion}).
\end{remark}

\begin{remark}[Scope of the direction map]
\label{rem:mapscope}
\Cref{thm:filterfirst,thm:bandlimited,cor:bandenergy} constrain the buffer $\Mbf_t$; their subspace-identification conclusions transfer to any map $\Mbf_t=\Ubf\Sigmabf\Vbf^\top\mapsto \Ubf\,h(\Sigmabf)\,\Vbf^\top$ with $h$ nonnegative and nondecreasing, since such a map carries $\Mbf_t$'s singular subspaces per index and preserves the singular-value ordering.
The family contains the raw buffer ($h(\sigma)=\sigma$), the powers $h(\sigma)=\sigma^{1-q}$, $q\in(0,1)$---single-step Shampoo \citep{gupta2018shampoo}, its preconditioners built from the current gradient alone, realizes them: exponent $-1/4$ per side is exactly the polar factor, smaller per-side exponents the intermediate powers---and the exact polar as its flattest member.
It does \emph{not} contain the deployed operator: $h_{\mathrm{NS}}$ is not monotone ($\psi'(1)=\psi_1+3\psi_3+5\psi_5\approx-0.72<0$, and $h_{\mathrm{NS}}'$ itself changes sign near $\sigma\approx0.006$), which is why \Cref{thm:nsupdate} replaces the ordering route by the Lipschitz route.
Trajectory-level consequences of the power family are outside this paper's scope (\Cref{sec:discussion}).
\end{remark}

Whenever momentum improves the signal-to-disturbance ratio before $\Dcal$---zero-mean noise, the Nyquist mode, or any band-limited disturbance above---$\Dcal(m_t)$ receives the cleaner input, and \Cref{thm:polarupdate,thm:nsupdate} state exactly when the cleaner input becomes a cleaner update.
\Cref{sec:matrix} tests the deterministic case at the buffer level, with a hill term $(-1)^t\Abf$ that is high-frequency but not noise, and overlays the constants of \Cref{thm:filterfirst} on the measured traces.
...
Each row: the result, its statement class (\Cref{sec:intro}), its falsifiable prediction, and the experiments that test it (E-labels as in the corresponding subsections; ``grid'' = the closed-loop grid of \Cref{sec:grid}).}
\label{tab:map}
\small
\begin{tabular}{l l p{5.6cm} l}
\toprule
result & class & prediction & test\\
\midrule
\Cref{prop:transfer}, \Cref{thm:filter} & exact & MSR tracks $\Hmag{\pi}^2$; exact transfer & E1, E4, E6\\
\Cref{rem:boundary} & exact & boundary weight $\Ocal(\Teff^2/T)$ & E4 (declared windows)\\
\Cref{thm:separation} & exact & hill peak at $\omega=\pi$, river at DC & E1, E4\\
\Cref{thm:spectrum}, \Cref{cor:ceiling} & exact & monotone spectrum; white ceiling $1/\Teff$; the high band is state, not residual & E3, E4, E12\\
\Cref{lem:confinement} & exact & high band appears at confinement & E6 (onset)\\
\Cref{prop:stability} & exact & divergence boundary at $\eta\lambda=2\Teff$ & E9, grid, E8, E11\\
\Cref{prop:variance}, \Cref{cor:hillloss} & exact & rms on the guide; hill loss falls by $\eta\lambda/2$ & E9, grid, E8\\
\Cref{cor:hb} & exact & at fixed $\eta_{\mathrm{HB}}$ the sweep reverses; optimum $\max(0,\sqrt{\eta_{\mathrm{HB}}\lambda}-1)$ & E9(f)\\
\Cref{prop:forced} & exact & tone response on $\lvert G_\beta\rvert$; passband $\beta$-free; resonance hurts & E13\\
\Cref{prop:curved} & exact (a,b); quasi-static (c) & confinement floor at $\eta\lambda(1+(a_{\mathrm{b}}k)^2)=2\Teff$; bend-corrected rms & E10, E8\\
\Cref{rem:burnin} & exact & tail loss turns up once $\Teff\sim T$ & E9, E7\\
\Cref{rem:window} & reading & window widens with $\lambda/\mu$, narrows with bend $k$ & grid, E10\\
\Cref{thm:filterfirst}, \Cref{thm:bandlimited} & exact (buffer) & pre-polar identifies the subspace; polar-only cannot (\Cref{prop:polaronly}) & E5, E7, E11\\
\Cref{thm:polarupdate}, \Cref{thm:nsupdate} & exact (update) & update error dies at $1/\Teff$ under the stated hypotheses; the knee of \Cref{rem:knee} & untested here (\Cref{sec:discussion})\\
\Cref{cor:bandenergy} & exact (buffer) & band energies suffice; HFER estimates $1-\gamma$ & E12\\
\Cref{rem:tradeoff} & reading & best $\beta$ interior: the U-shape & E2, E7, E10\\
\bottomrule
\end{tabular}
\end{table}
...
\end{itemize}
\paragraph{Statistics and conventions.}
Seed counts, window types, scored windows, and burn-in are declared per experiment; error bars are seed standard errors unless a caption states otherwise.
Divergence counts as failure in every criterion: a diverged seed is never dropped from a mean, a cell containing one is never the baseline of a quoted percentage, and diverged cells are reported as such (E9, E11).
Scored windows are long relative to the filter memory wherever a stationary formula is quoted (E9: at least $7.5\,\Teff$ at every $\beta$); where $\Teff$ approaches the window, the finite-window behavior is itself the prediction under test (\Cref{rem:burnin}).
The high band is $\omega\ge0.6\pi$, fixed once here and never tuned per experiment; spectra use rectangular windows, matching the finite-window statements, except where a caption declares a Hann taper (E4).
\paragraph{Reproducibility.}
Every number in this paper is read from a cached run record (\texttt{config.json}, \texttt{metrics.json}, \texttt{arrays.npz}) keyed by the run identifier quoted in its caption; figures render from those records alone, never from re-runs.
The code (\texttt{core/} library, run drivers, figure modules), the cached records, and per-figure provenance will be released as a public repository, version-tagged at submission.
...
At fixed $\eta_{\mathrm{HB}}$ with $\eta_{\mathrm{HB}}\lambda=0.6$---the strictly increasing regime of \Cref{cor:hb}---the composed formula predicts, and the run measures, tail hill loss \emph{increasing} in $\beta$ (Spearman $+1.00$, on its guide in $10$ of $10$ cells, $0.086$ to $13.2$), while the EMA sweep sharing the same $\beta=0$ cell decreases ($0.086$ to $0.060$).
...
These floors are predicted by the frozen diagnostic: the \Cref{prop:curved} worst-case threshold $\eta\lambda(1+(a_{\mathrm{b}}k)^2)<2\Teff$ reproduces the measured floor exactly in $7$ of the $10$ cells and to one grid step in the other $3$, with misses in both directions---a quasi-static stability diagnostic read against a noise-driven escape criterion.
Second, the window's floor-to-top span narrows, from six grid steps at $k=0.3$ to four ($\eta\lambda=1.8$) and three ($\eta\lambda=2.5$) at $k=1.8$---the curvature counterpart of the widening with conditioning in \Cref{sec:grid}; the joint escape-and-lag criterion is stricter still, and its collapse at $k=1.8$ is quantified below.
...
\subsection{Implementation checks (E1--E4)}
\label{sec:implchecks}
Four controlled checks verify the exact identities on the simulators that instantiate them; they are reported in full in \Cref{app:impl}: the straight- and curved-valley trajectory families (E1, E2), gradient-noise filtering against the $1/\Teff$ floor (E3), and the per-bin transfer ratio against $\Hmag{\omega}$ (E4).
They are implementation checks in the sense of \Cref{tab:map}---they establish that the simulators realize the formulas (per-bin transfer ratio on $\Hmag{\omega}$ to relative error $0.001$ on synthetic streams; MSR on the $\Hmag{\pi}^2$ guide; NSR on the $1/\Teff$ floor; hill spectra peaked at $\omega=\pi$)---not independent evidence about neural networks.
Their one forward-pointing observation: the open-loop attenuation and the closed-loop floor distance are optimized at different $\beta$ (E1; \Cref{fig:e8}c), the earliest form of the window story that E9, E10, and E13 test at trajectory level.
...
Second, the momentum benefit follows the regime scaling of \Cref{cor:hillloss}: on the three learning rates where every arm survives all seeds, the best-$\beta$ improvement in tail training loss over $\beta=0$ rises $0.8\%$, $3.5\%$, $11.9\%$.
At $\mathrm{lr}=0.4$ the comparison changes kind rather than degree: counting divergence as failure, the $\beta=0$ arm fails (one of three seeds diverges; its surviving-seed mean $2.488$ is reported for context only, and no percentage is quoted against it), while the best cell of the whole sweep is the stabilized $(\mathrm{lr}=0.4,\beta=0.95)$ at $1.946$.
...
(a) tail training loss (mean of the last $200$ steps) versus $\beta$ per learning rate; error bars seed std; the $(\mathrm{lr}{=}0.4,\beta{=}0)$ point is a surviving-seed mean over $2$ of $3$ seeds and is the one cell counted as failed in the body's divergence-counted criterion; legend entries carry the raw-stream HFER at $\beta=0$.
(b) seed-0 loss curves at $\mathrm{lr}=0.4$; $\times$ marks the $\beta=0$ divergence.
(c) best-$\beta$ benefit over $\beta=0$ (bars) for the learning rates whose $\beta=0$ arm survives all seeds, and HFER$(\Gbf,\beta{=}0)$ (line) versus learning rate, white-stream baseline dashed; at $\mathrm{lr}=0.4$ the $\beta=0$ baseline fails (one of three seeds) and no percentage is drawn.
...
Two classical views of momentum meet in the temporal filter; the third is complementary, not identical.
Denoising is the filter on a broadband stream---the $1/\Teff$ attenuation of the white floor (\Cref{cor:ceiling})---and this paper's reading is the filter fed by the landscape: \emph{structured} high-frequency components, of which the river-valley hill is the canonical example.
Acceleration is not delivered by open-loop filtering: under EMA normalization the DC gain is exactly $\Hmag{0}=1$---a persistent gradient is passed, not amplified---and the $1/(1-\beta)$ gain of the heavy-ball convention is the normalization fact of \Cref{sec:ema}, not a rate mechanism.
Condition-number acceleration arises from the tuned second-order dynamics of the coupled iterate--buffer system \citep{polyak1964momentum,sutskever2013momentum,lessard2016analysis}; the AR(2) of \Cref{eq:closedloop} is that system in the hill coordinate, so the two accounts share hardware while answering different questions---rate of contraction there; stationary error, stability margin, and frequency selectivity here.
\Cref{thm:filter} states the filtering--lag pair with explicit constants at the stream level; \Cref{prop:stability,prop:variance} carry them to the trajectory, where the benefit is a large-learning-rate effect under sustained excitation and the deterministic transient remains the caveat; and \Cref{prop:forced} completes the trajectory story frequency by frequency---removed at Nyquist, neutral in the passband, amplified at the loop's own mode---the three arms E13 measures on parameter-free guides.
...
The direct sweep (E9) measures the improvement side of this statement at the predicted $\eta\lambda/2$; the curved valley (E2), the instantaneous-target reading of E3, the regime map (E8), and the bend-frequency sweep (E10) show the window side---its floor predicted by \Cref{prop:curved}'s frozen diagnostic---including the closed-loop caveat that curvature is paid partly in river speed rather than lag.
On schedules, the closed-loop reading yields a design constraint rather than a documented result: the confinement floor---the smallest $\beta$ that keeps the trajectory in the tube---is a lower bound any $\beta$ schedule must respect, and the confinement onset (E6) supports raising $\beta$ once the iterate is confined, so when to raise $\beta$ is a confinement question, not a clock question.
A systematic study of $\beta$ schedules against the floor is left to separate work.
...
\paragraph{Limitations.} The closed-loop propositions are proved for the linear (straight-valley) hill; the curved valley now has \Cref{prop:curved}---exact for the linear floor, where the loop is two straight loops at the Hessian eigenvalues, and quasi-static for general floors with its two hypotheses stated and E10-calibrated---but a global curved-valley theorem remains open, and the quasi-static validity margin $k\vriv\,\Teff\lesssim1$ is itself the window's top edge.
\Cref{thm:filterfirst,thm:bandlimited,cor:bandenergy} are statements about the buffer feeding the direction map, and \Cref{thm:polarupdate,thm:nsupdate} carry the recovery to the map's output under their stated hypotheses (full rank, or the deployed Newton--Schulz operator with its Lipschitz price)---none are nonlinear trajectory theorems, and all exclude Nesterov momentum, as does \citet{li2026denoise}; the Muon comparisons here probe one matrix in small models with the exact polar, so the deployed-operator constants ($L_h$, the knee of \Cref{rem:knee}) and practical Muon training (all matrix layers, tuned baselines, Newton--Schulz at scale) remain unmeasured.
...
\Cref{prop:variance} assumes additive, state-independent white noise, which mini-batch noise is not: E12 verifies the temporal whiteness of the sampling residual at this scale, not state-independence or isotropy, so the closed-form constants remain model statements there; whiteness itself is read from the flat spectrum and HFER at the single predeclared cutoff---autocorrelation-test batteries and cutoff sweeps are not run at this scale.
Production scale, Adam-preconditioned updates, learning-rate schedules, and trajectory-level behavior of the \Cref{rem:mapscope} family remain untested.
...
\begin{proof}[Proof of \Cref{thm:filterfirst}]
...
\begin{proof}[Proof of \Cref{thm:bandlimited}]
...
\begin{proof}[Proof of \Cref{cor:bandenergy}]
...
\begin{proof}[Proof of \Cref{lem:oddlip}]
Write $H(\Xbf)=\bigl(\begin{smallmatrix}\zerobf&\Xbf\\\Xbf^\top&\zerobf\end{smallmatrix}\bigr)$, the Hermitian dilation.
Odd powers preserve the off-diagonal structure, $H(\Xbf)^{2j+1}=H\bigl(\Xbf(\Xbf^\top\Xbf)^{j}\bigr)$, so $h(H(\Xbf))=H(h(\Xbf))$ for every odd polynomial $h$, and the eigenvalues of $H(\Xbf)$ are $\pm\sigma_i(\Xbf)$ together with $\lvert d_1-d_2\rvert$ zeros---all in $[-s,s]$ when $\lVert\Xbf\rVert_2\le s$.
Now let $\Pbf,\Qbf$ be Hermitian with spectra in $[-s,s]$ and spectral decompositions $\Pbf=\sum_i\lambda_iP_i$, $\Qbf=\sum_j\mu_jQ_j$.
The blocks $\Zbf\mapsto P_i\Zbf Q_j$ are Frobenius-orthogonal, and
\[
P_i\bigl(h(\Pbf)-h(\Qbf)\bigr)Q_j=\bigl(h(\lambda_i)-h(\mu_j)\bigr)P_iQ_j,
\qquad
P_i(\Pbf-\Qbf)Q_j=(\lambda_i-\mu_j)\,P_iQ_j,
\]
so the scalar bound $\lvert h(\lambda_i)-h(\mu_j)\rvert\le L\lvert\lambda_i-\mu_j\rvert$ controls each block, and summing squares over $(i,j)$ gives $\lVert h(\Pbf)-h(\Qbf)\rVert_F\le L\lVert\Pbf-\Qbf\rVert_F$---the divided-difference bound, classical in the Hilbert--Schmidt norm \citep{bhatia1997matrix}.
Apply it to $\Pbf=H(\Xbf)$, $\Qbf=H(\Ybf)$; the factor $\lVert H(\Xbf)-H(\Ybf)\rVert_F=\sqrt2\,\lVert\Xbf-\Ybf\rVert_F$ appears on both sides and cancels.
\end{proof}

\section{Implementation Checks (E1--E4)}
\label{app:impl}

The four checks summarized in \Cref{sec:implchecks}, with their figures and tables; measurements as defined in \Cref{sec:measurements}.

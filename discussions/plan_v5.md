...
- **W4 E7 schedule claims (R4).** Default: **remove** the p. 32 quantitative schedule claims (factors 15/118, detector steps) from the Discussion, replace with one qualitative sentence + pointer to future work — the material is the BETASCHEDULER paper's opening experiment, and paper 1 shouldn't carry undocumented numbers for it. Branch (Leon's call, §8): document instead as an appendix subsection rendered from the existing cached E7 runs, +1.5 pages. Either way the current state (numbers with no methodology section) cannot survive.
...
## 8. Decisions for Leon (defaults applied unless overridden)

1. **E7 schedule claims:** remove-and-defer to the scheduler paper (default) vs. document as appendix from cached runs.
2. **Adam scope:** keep Adam-preconditioned streams out of headline claims, one exploratory subsection at most (default), vs. full Adam arm in G4 (+~60 A100-h and a theory-scope headache).
3. **CIFAR-100 + BN-free control:** headline cells only (default) vs. full grids (+~40 h).
4. **Public repo timing:** at resubmission (default) vs. immediately.
5. **124M confirmation token budget:** 2.5B (default, ~Chinchilla-20×) vs. 5B tokens (2× cost, tighter curves).

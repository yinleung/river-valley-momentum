...
- **W4 E7 schedule claims (R4) — BLOCKED on §8 item 1 (Leon deferred the decision 2026-07-19; open to-do).** Two prepared branches: (a) **remove** the p. 32 quantitative claims (factors 15/118, detector steps) from the Discussion, replace with one qualitative sentence + future-work pointer — the material is the BETASCHEDULER paper's opening experiment; (b) **document** as an appendix subsection rendered from the existing cached E7 runs (+~1.5 pages). Either way the current state (numbers with no methodology section) cannot survive, so obtain Leon's decision at W4 kickoff at the latest, before the final W-pass.
...
1. **E7 schedule claims — DECISION DEFERRED (Leon, 2026-07-19); open to-do.** Neither branch is chosen: (a) remove the p. 32 quantitative claims and defer the material to the scheduler paper, or (b) document them as an appendix subsection rendered from the cached E7 runs (+~1.5 pages). The executing agent must NOT act on W4 until Leon decides; the review's hard constraint stands either way (undocumented numbers cannot remain), so this to-do blocks the final W-pass — re-raise with Leon at W4 kickoff at the latest.
